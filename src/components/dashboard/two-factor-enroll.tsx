...
function Button({ children, onClick, disabled = false, className = '', autoFocus }: ButtonProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      autoFocus={autoFocus}
      className={`w-full py-6 px-6 rounded-2xl font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus-visible:ring-2 focus-visible:ring-cyan-400 ${className} ${
...
export default function TwoFactorEnroll({ onComplete }: { onComplete?: () => void }) {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    const pastedCode = pastedData.replace(/[^0-9]/g, '').slice(0, 6);
    if (pastedCode) {
      setCode(pastedCode);
      if (pastedCode.length === 6) {
         // Auto-verify optionally, but we leave it for user to click
      }
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-3xl p-8" role="region" aria-label="Two-Factor Authentication Enrollment">
      ...
      {step === 'intro' && (
        <>
          <p className="helper-text mb-8 text-center text-slate-300">We&apos;ll guide you through setting up 2FA using a time-based one-time password app.</p>
      ...
      {step === 'qr' && (
        <div className="text-center">
          <div className="mx-auto w-56 h-56 bg-white rounded-2xl flex items-center justify-center mb-6 border-8 border-slate-800" aria-label="QR Code">
            <div className="text-slate-900 font-mono text-xs">SCAN QR CODE WITH AUTHENTICATOR APP</div>
          </div>
          <p className="helper-text mb-6 text-slate-300">Scan this QR code with Google Authenticator, Authy, or another TOTP app.</p>
      ...
      {step === 'recovery' && (
        <div>
          <h3 className="text-lg font-semibold mb-4 text-slate-100">Save Your Recovery Codes</h3>
          ...
          <div className="bg-slate-950 p-6 rounded-xl border border-slate-700 mb-6">
            <div className="flex justify-between items-center mb-4">
              <span className="font-medium text-sm text-slate-200">Recovery Codes</span>
              <button 
            ...
            <div className="grid grid-cols-2 gap-4 font-mono text-sm">
              {recoveryCodes.map((c, index) => (
                <div key={index} className="bg-slate-900 py-2 px-4 rounded border border-slate-800 text-center tracking-widest text-slate-300" aria-label={showCodes ? `Recovery code ${index + 1}: ${c}` : `Recovery code ${index + 1} hidden`}>
                  {maskRecoveryCode(c, showCodes)}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 mb-6">
            <input 
              ...
              className="mt-1 w-4 h-4 rounded border-slate-600 text-cyan-500 focus:ring-cyan-500 bg-slate-900 focus:outline-none focus-visible:ring-2"
            />
          ...
          <Button 
            ...
            className="bg-cyan-500 hover:bg-cyan-400 text-black disabled:bg-slate-700 disabled:text-slate-500"
          >
            Complete Setup
          </Button>
        </div>
      ...
      {step === 'success' && (
        <div className="text-center py-8">
          ...
          <h3 className="text-2xl font-semibold mb-3 text-slate-100">2FA Enabled Successfully</h3>
