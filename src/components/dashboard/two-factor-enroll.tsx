...
export default function TwoFactorEnroll({ onComplete }: { onComplete?: () => void }) {
  ...
  const [recoveryKey] = useState(() =>
    'CP-' + Array.from({ length: 32 }, () => Math.random().toString(36)[2]).join('').toUpperCase()
  );
    ...
    <div className="max-w-md mx-auto bg-slate-900 border border-slate-700 rounded-3xl p-8">
      ...
      {step === 'intro' && (
        <>
          ...
          <EnrollButton onClick={() => setStep('qr')} className="bg-cyan-500 hover:bg-cyan-400 text-black">Begin Setup</EnrollButton>
        </>
      )}
      ...
      {step === 'qr' && (
        <div className="text-center">
          ...
          <EnrollButton onClick={() => setStep('verify')}>I Have Scanned It</EnrollButton>
        </div>
      )}

      {step === 'verify' && (
        <div>
          <label className="block text-sm mb-3 font-medium" htmlFor="two-factor-code">Enter the 6-digit code from your authenticator app</label>
          <input
            id="two-factor-code"
            type="text"
            maxLength={6}
            ...
            placeholder="000000"
          />
          <EnrollButton onClick={handleVerify} disabled={code.length !== 6} className="mt-6 bg-cyan-500 hover:bg-cyan-400 text-black">
            Verify Code
          </EnrollButton>
        </div>
      )}
      ...
      {step === 'recovery' && (
        <div>
          ...
          <EnrollButton onClick={downloadRecovery} className="mb-4 bg-white text-black hover:bg-white/90">Download Recovery Key</EnrollButton>
      ...
      {step === 'success' && (
        <div className="text-center py-8">
          ...
          <EnrollButton onClick={onComplete} className="bg-cyan-500 hover:bg-cyan-400 text-black">Return to Settings</EnrollButton>
        </div>
      )}
    </div>
  );
